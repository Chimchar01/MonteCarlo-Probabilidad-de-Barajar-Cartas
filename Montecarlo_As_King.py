...
#Encontrar un as y un rey seguidos del mismo palo. Probabilidad analítica: 7.692308
def As_King (deck):
    condition = False
...
#Encontrar dos reys consecutivos. Probabilidad analítica: 21.737557
def King_King (deck):
    for i in range(len(deck)-1):
...
##############################################################################
##############################################################################
#Estas son las funciones del programa Monte Carlo
...
def D_frame (trials_list,values_list,values_analytic,deviation_list,dif_list):
    ...
    df.to_excel('Estadísticas Juego de Cartas.xlsx')
...
def Graph (trials_list, values_list,realvalue):
    ...
    ax.set_title('Probabilidad con Monte Carlo')
    ...
    plt.savefig('Probabilidad Juego de Cartas.jpg')
    plt.show()
##############################################################################
...
#Corre el programa
def Run(game):
    values_list, deviation_list = Deviation_Multirun(runs,trials_list,deck,game)
...
trials_list = [10,100,1000] #Lista con número de veces que se baraja el deck
...
Run(As_King) #Activa las funciones del programa
